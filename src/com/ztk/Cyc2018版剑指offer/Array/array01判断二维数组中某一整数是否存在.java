...
/**
 * （1）题目：在一个二维数组中，每一行都按照从左到右递增的顺序排序，每一列都按照从上到下递增的顺序排序。
...
public class array01判断二维数组中某一整数是否存在 {
    public boolean Find(int target,int[][] array){//array 待查找的数组,target 要查找的数
        if(array == null || array.length == 0 || array[0].length == 0)//数组为空时则直接false，表示没找到
            return false;//查找结果，true找到，false没有找到

        int rows = array.length;// 二维数组的行数
        int cols = array[1].length; //二维数组的列数
        ...
        while(r<= rows - 1&& c >=0){
            if(target == array[r][c])// array[r][c]即为array[0][cols-1]，即右上角那个数
                return true;//如果找到了就直接退出
            else if(target>array[r][c])// 如果要找的数比找到的数大，说明要找的数在当前数的下边
                r ++;// 行数加一，代表向下移动
