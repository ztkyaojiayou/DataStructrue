...
/**
 * 题目：从上往下打印出二叉树的每个结点
 *      （也即：按层打印，且同一层的结点按照从左向右的顺序打印）
 *
 * 【解】：与题15和17的思路完全一样，代码上也只在关键处有几行之差，注意对比理解
 ...
 *
 * 3.添加元素到队列
 * offer()　将指定的元素插入此队列（的末尾）（如果立即可行且不会违反容量限制），插入成功返回 true；否则返回 false。当使用有容量限制的队列时，offer方法通常要优于 add方法——add方法可能无法插入元素，而只是抛出一个  IllegalStateException异常
