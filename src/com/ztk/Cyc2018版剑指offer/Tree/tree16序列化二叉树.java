 ...
 * （2）反序列化二叉树：
 * 定义一个索引index，将字符串根据逗号分割为数组（要用到String类的split方法），根据index的值来设置树节点的val，若节点的值为#，则返回 空的树节点。
 *
 */
...
public class tree16序列化二叉树 {

    int index = -1;//这个变量虽然只有在反序列化方法中，但也必须放在所有方法的外面，把其定义为全局变量
        /**
         * 1.序列化(使用的是递归，非常简单）
         * 分别递归遍历左节点和右节点，空使用 # 代替，节点之间用 ，隔开
         */
        public String Serialize(TreeNode16 root) {//传入根节点（在牛客网上，对于tree，都是先传入的根节点，后面再利用递归或left/right指针的手法处理其他节点）
        ...
        TreeNode16 Deserialize(String str) {
            ...
            if (!s[index].equals("#")) {//若不是叶子节点，则通过递归，把其左右节点都安排上（若是叶子节点，则之即返回即可）
