 ...
 * 这个调整过程需要两个指针辅助：pre记录当前节点的前一个结点的位置，好让该结点的next指针指向前一个结点，但是在指向前一个结点前需要用一个指针next记录后一个结点地址，避免结点丢失。
 *
 * 具体做法：以3个结点的链表为例：
 * （1）用pre记录当前节点的前一个节点，用next记录当前节点的后一个节点
 *
 * （2）当前节点head不为空，进入循环，先记录head的下一个节点位置next = b;再让head的指针指向pre
 *
...
public class Linkedlist30反转链表 {
    ...
    public ListNode30 ReverseList(ListNode30 head) {
        ...
        ListNode30 pre = null; // 用于表示当前节点的前一个节点，由于一开始是头结点head，因此先指定为null
        ListNode30 next = null; // 用于表示当前节点的下一个节点

        while( head != null){//一直向右走，每反转一个结点就向右走，一直到遍历完整个链表
            //3.开始反转（核心代码）
            next = head.next; // 记录当前节点的下一个节点位置，等会要用（用于head指针右移（line48））
            head.next = pre; // 让当前节点指向前一个节点位置，完成反转

            //3.1反转完一个结点之后，把pre和head结点均右移，处理下一个结点
            pre = head; // pre指向head结点，也即往右走，开始反转下一个结点
            head = next;// 当前节点head也往右继续走，开始反转下一个结点
        }
